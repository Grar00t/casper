...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --smoke)    RUN_SMOKE=1 ;;
        --bench)    RUN_BENCH=1 ;;
        -h|--help)  echo "usage: scripts/build.sh [--debug|--release] [--arch generic|x86_64|arm64] [--compiler CC] [--lint] [--smoke] [--bench]"; exit 0 ;;
...
if [[ "$CONFIG" == "release" ]]; then
    ...
    OPT="-O0 -g3 -DDEBUG"
    LDFLAGS="-lm"
    if echo 'int main(void){return 0;}' | "$CC" -fsanitize=address,undefined -x c -c - -o /dev/null >/dev/null 2>&1; then
...
if [[ "$RUN_LINT" == "1" ]]; then
    command -v cppcheck >/dev/null 2>&1 || { echo "[build] cppcheck not found" >&2; exit 1; }
    echo "-- cppcheck ---------------------------------"
    cppcheck \
        --error-exitcode=1 \
        ...
        -I "$INCLUDE" -I "$CORE" \
        "$ROOT/tokenizer.c" \
        "$CORE/niyah_core.c" "$CORE/niyah_init.c" "$CORE/niyah_main.c" \
...
build_target trainer \
    "$CORE/niyah_train.c" "$CORE/niyah_core.c" "$CORE/niyah_init.c" "$ROOT/tokenizer.c"

build_target niyah_hybrid \
    "$CORE/niyah_hybrid_main.c" "$CORE/niyah_core.c" "$CORE/niyah_init.c" \
...
if [[ -f "$CORE/bench_niyah.c" ]]; then
    build_target bench_niyah "$CORE/bench_niyah.c" "$CORE/niyah_core.c" "$CORE/niyah_init.c"
fi
